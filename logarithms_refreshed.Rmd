...
The value of the exponent (in this case 2) tells us the number of times 1 gets multiplied by the base.
...
```{python}
# one divided by 10 three times
((1/10)/10)/10
```

For any base with a negative exponent, we can also write it as 1 divided by the base with a positive exponent of the same value e.g.:
...
Once we have transformed two numbers to logs, if we subtract the logs
, then this is the same as performing division on the original numbers.
...
1 is the first element in the `arr` array above. `np.log(arr)` transforms the first element into a 0. The answer will be 0 whenever 1 is log-transformed, for instance in the first step of the log-add-unlog trick. Remember that the log of 1 is asking the question "what power would I have to raise the base to, in order to get 1?". 
