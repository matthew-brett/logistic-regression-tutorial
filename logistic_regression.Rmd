...
In our dataset the event of interest is a patient having a 'good' appetite, and the event of no interest is having a 'poor' appetite. So the odds ratio of having a good appetite is:

$$\frac{139}{19} = 7.33 = \frac{\text{number of patients with good appetite}}{\text{number of patients with poor appetite}} $$.

Based on this odds ratio, it is 7.33 times more likely that a patient selected at random would have a "good" appetite rather than "poor" appetite.

We can also express the odds ratio via the proportion of events, rather than the number of events:
...
The odds ratio is the same either way, whether we express it by the proportion of events or by the number of events.

The odds ratio is just a way of saying a probability in a different way, and we can convert easily between probabilities and odds ratios.
...
To illustrate how the odds ratio changes as the number of events of interest/not of interest changes; the dataframe below some possible ways our 158 patients' data could have looked, and what the odds ratio would be in each instance. 

The first row of the dataframe shows what the odds ratio of "good" appetite would be if all 158 patients had "poor" 
appetite. 

The final row of the dataframe shows what the odds ratio of "good" appetite would be if all 158 patients 
had "good" appetite. The rows in between so show odds ratios between these extremes:
...
The output of the cell below shows the verbal interpretation of row 6 of the dataframe above. If you are using an interactive version of this notebook, to interpret a different row of the dataframe, change the value of `row` and re-run the cell below:
...
We have seen that `odds_ratio = p / (1 - p)`. If there are no events of interest, then `p == 0` and the odds ratio is 0, as you can see from the first row of the dataframe above. 

If all of the events are events of interest (as in the final row of the dataframe above), then `p == 1` and this gives:
...
predictions, on the corresponding sigmoid, and compares these predictions to the 0 / 1 values in `appetite_dummy`.
...
<!-- #region -->
Let's take a step back and assess what's going on here. By default, logistic regression predicts the probability that a given observation falls into class 1 of the binary outcome variable. For our dataset, this is the probability that an observation has an `appetite_dummy` score of 1 (e.g. the probability that a patient has "good" appetite). These predictions are contained in the `pp_of_1` column. 

By contrast, rather than just showing us the probability of a label of 1, the `pp_of_correct_label` column tells us the predicted probability of the `appetite_dummy` score that each patient actually has:

* If the patient actually has a "good" appetite (e.g. their `appetite_dummy` label is 1), then the `pp_of_correct_label` shows the predicted probability of an `appetite_dummy` score of 1. 


* If the patient has a "poor" appetite, (e.g. their `appetite_dummy` label is 0), then the `pp_of_correct_label` shows the predicted probability of an `appetite_dummy` score of 0. 


There's a fancy short-cut to the several lines of code in the cell above, that relies on the fact
that multiplying by 0 gives 0.  We can form the `pp_of_correct_label` column by first
multiplying the 0 / 1 label column by the `pp_of_1`.  This correctly sets the
`pp_of_correct_label` values for labels of 1, and sets the remainder to 0.  Then we
reverse the 0 / 1 labels by subtracting from 1, and multiply by `pp_of_0` to
set the 0 values to their correct values.  Adding these two results gives us
the correct set of values for both 1 and 0 labels.
<!-- #endregion -->
...
When the `pp_of_correct_label` values are near 1, this means the result was close to the prediction.  When they are near 0, it means the result was unlike the prediction.
...
print('Adding, inverting the logs', np.exp(np.log(11) + np.log(15)))
```
