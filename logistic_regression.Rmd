...
first pair `p1` and the second pair `p2`.  Call the likelihoods for `p1` and
`p2`: `L2` and `L2` respectively.  Say we've got another way of calculating a
value for `p1` and `p2`, and the values that come back from the new calculation
are `N1` and `N2` respectively.  We can swap this new way into our cost
function, and get the same parameters back from `miminize`, as long as it is
always true that:
