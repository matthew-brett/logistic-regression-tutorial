...
disease](https://www.sciencedirect.com/science/article/abs/pii/S0270929508001666).  
In our case, we wonder whether the extent of kidney damage does a convincing job 
in predicting whether the patient has a "good" appetite.
...
appetite_dummy = hgb_app['appetite_dummy']
```

Why not use the same linear regression technique for our case?  After all, the
`appetite_dummy` values are just numbers (0 and 1), as are our `hemoglobin` values.
...
158 rows means there are 158 "observations" (patients), so there will be the same number of scores on the
...
\begin{bmatrix}
         ...
         \end{bmatrix} = intercept + slope *\begin{bmatrix}
           {11.2} \\
           {9.5} \\
           {10.8} \\
           \vdots \\
...
Let's use linear regression on the current example.  From looking at our plot above, 
we start with a guess of -1 for the intercept, and 0.1 for the slope.

```{python}
# Use minimize to find the least sum of squares solution.
min_lin_reg = minimize(ss_any_line, [-1, 0.1], args=(hemoglobin, appetite_dummy))
...
```
predicted_appetite_dummy = intercept + slope * hemoglobin
```
...
```{python tags=c("hide-cell")}
# Do the base plot of the hemoglobin and appetite_dummy.
plot_hgb_app()
...
several unhappy features for our task of predicting the `appetite_dummy` 0 / 1
values.

One thing to like about the line is that the predictions are right to suggest
that the value of `appetite_dummy` is more likely to be 1 (meaning "good") at
...
starts to predict a value for `appetite_dummy` that is greater than 1 - which, of
course, cannot occur in the `appetite_dummy` values, which are restricted to 0 or
1.

Looking at the plot, without the regression line, it looks as if we can be
fairly confident of predicting a 1 ("good") value for a hemoglobin above 12.5, 
but we are increasingly less confident about predicting a 1 value as hemoglobin 
drops down to about 7.5, at which point we become confident about predicting a 0 
value.
...
Here's another prediction line we might use for `appetite_dummy`, with the
predicted values.
...
range of the `appetite_dummy` data it is trying to predict.  It climbs steeply to a
...
predicted y (`appetite_dummy`) value of about 0.73.  We can interpret this
...
appetite (`appetite_dummy` value of 1).

Now let us say that we would prefer to use this kind of sigmoid line to predict
`appetite_dummy`.  So far, we have only asked `minimize` to predict directly from a
...
We will explain more about the two stages of the transform below, but for now, here 
are the two stages in action.
...
random from the data frame, and record their `Appetite`, then we expect the 
proportion of "good" values in the long run, to be 139 / 158 --- about 0.88.  
That is the same as saying there is a probability of 0.88 of a randomly-drawn 
patient of having a "good" appetite.
...
$$\frac{139}{158} = 0.88 = \frac{\text{number of patients with good appetite}}{\text{total number of patients}}$$.
...
the odds are 5 to 1 that it will rain today.  We mean that it is five times more 
likely that it will rain today than that it will not rain today.  On another day 
we could estimate that the odds are 0.5 to 1 that it will rain today, meaning that 
it is twice as likely *not* to rain, as it is to rain.

The odds ratio is the number of times we expect to see the event of interest (e.g. rain) 
for every time we expect to see the event of no interest (not rain). 

In our dataset the event of interest is a patient having a 'good' appetite, and 
the event of no interest is having a 'poor' appetite. So the odds ratio of having a good appetite is:

$$\frac{139}{19} = 7.33 = \frac{\text{number of patients with good appetite}}{\text{number of patients with poor appetite}} $$.

Based on this odds ratio, it is 7.33 times more likely that a patient selected 
at random would have a "good" appetite rather than "poor" appetite.

We can also express the odds ratio via the proportion of events, rather than the 
number of events:

$$\frac{139/158}{19/158} = 7.33 = \frac{\text{proportion of patients with good appetite}}{\text{proportion of patients with poor appetite}} $$.

The odds ratio is the same either way, whether we express it by the proportion of 
events or by the number of events.

The odds ratio is just a way of saying a probability in a different way, and we 
can convert easily between probabilities and odds ratios.

To convert from a probability to an odds ratio, we remember that the odds ratio 
is the number of times we expect to see the event of interest for every time we 
expect to see the event of no interest.   This is the probability (proportion) 
for the event of interest, divided by the probability (proportion) of the event 
of no interest.  Say the probability p is some value (it could be any value):
<!-- #endregion -->
...
To illustrate how the odds ratio changes as the number of events of interest/not of 
interest changes; the dataframe below some possible ways our 158 patients' data could 
have looked, and what the odds ratio would be in each instance. 

The first row of the dataframe shows what the odds ratio of "good" appetite would be 
if all 158 patients had "poor" appetite. 

The final row of the dataframe shows what the odds ratio of "good" appetite would be 
if all 158 patients had "good" appetite. The rows in between so show odds ratios between 
these extremes:

```{python}
# ignore this code it just generates the dataframe example below
good = np.array([])
poor = np.array([])
n = len(hgb_app)
for i in np.arange(158, step = 10):
    num_good = i
    good = np.append(good, num_good)
    poor = np.append(poor, n - num_good)    
num_good = 158
good = np.append(good, num_good)
poor = np.append(poor, n - num_good)
odds_ratios =  good/poor
illus = pd.DataFrame({'num. patients with good appetite': good.astype('int'),
             'num. patients with poor appetite': poor.astype('int'),
             'odds ratio': np.round(odds_ratios,2)})
illus
```

The output of the cell below shows the verbal interpretation of row 6 of the dataframe 
above. If you are using an interactive version of this notebook, to interpret a different 
row of the dataframe, change the value of `row` and re-run the cell below:

```{python}
# set the row of the dataframe above that you want to interpret (up to 16)
row = 6
    
# ignore this code, it just generates the printout
if illus['odds ratio'].iloc[row] > 1:
        print('\nIf there are '+str(illus['num. patients with good appetite'].iloc[row])+' patients with "good" appetite, and '+str(illus['num. patients with poor appetite'].iloc[row])+' patients with "poor" appetite, then the odds ratio is '+str(illus['odds ratio'].iloc[row])+'.')
        print('\nThis means that "good" appetite is '+str(illus['odds ratio'].iloc[row])+' times more likely than "poor" appetite.')
if illus['odds ratio'].iloc[row] < 1:
        print('\nIf there are '+str(illus['num. patients with good appetite'].iloc[row])+' patients with "good" appetite, and '+str(illus['num. patients with poor appetite'].iloc[row])+' patients with "poor" appetite, then the odds ratio is '+str(illus['odds ratio'].iloc[row])+'.')
        print('\nThis means that "good" appetite is '+str(illus['odds ratio'].iloc[row])+' times as likely as "poor" appetite.')
```

We have seen that `odds_ratio = p / (1 - p)`. If there are no events of interest, 
then `p == 0` and the odds ratio is 0, as you can see from the first row of the 
dataframe above. 

If all of the events are events of interest (as in the final row of the dataframe 
above), then `p == 1` and this gives:
...
values to odds ratios for our appetite predications, we get the following:
...
```{python}
# some values
original_vals = np.array([1, 0.5, 3, 6, 0.1])

original_vals
```

```{python}
# log-transforming these values
log_vals = np.log(original_vals)
log_vals
```

```{python}
# np.exp reverses the effect of np.log, it restores the logged values back to the original values
vals_back = np.exp(log_vals)
vals_back
```
...
predictions, on the corresponding sigmoid, and compares these predictions to the 
0 / 1 values in `appetite_dummy`.
...
```{python}
sigmoid_error = appetite_dummy - sigmoid_predictions
...
```{python}
ss_logit([-7, 0.8], hemoglobin, appetite_dummy)
```
...
```{python}
min_res_logit = minimize(ss_logit, [-7, 0.8], args=(hemoglobin, appetite_dummy))
min_res_logit
```
...
```{python}
# A pandas series containing the predicted probability of 0 (poor appetite) for each patient.
pp_of_0 = 1 - pp_of_1

# The next line sets all the label == 1 values correctly, but
# the label == 0 values are now incorrect (e.g this array current contains only the probability of label == 1).
pp_of_correct_label = pp_of_1.copy()

# Create a Boolean array which is True where appetite_dummy == 0, e.g. True for patients with "poor" appetite
poor_appetite = appetite_dummy == 0

# for the patients who actually had a 0 label ("poor" appetite), get the predicted probability of a 0 label
pp_of_correct_label[poor_appetite] = pp_of_0[poor_appetite]

# Put this into the data frame for display
hgb_predicted['pp_of_correct_label'] = pp_of_correct_label
hgb_predicted
```

<!-- #region -->
Let's take a step back and assess what's going on here. By default, 
logistic regression predicts the probability that a given observation 
falls into class 1 of the binary outcome variable. For our dataset, this 
is the probability that an observation has an `appetite_dummy` score of 
1 (e.g. the probability that a patient has "good" appetite). These 
predictions are contained in the `pp_of_1` column. 

By contrast, rather than just showing us the probability of a label 
of 1, the `pp_of_correct_label` column tells us the predicted probability 
of the `appetite_dummy` score that each patient actually has:

* If the patient actually has a "good" appetite (e.g. their `appetite_dummy` 
    label is 1), then the `pp_of_correct_label` shows the predicted probability 
    of an `appetite_dummy` score of 1. 
    
    
* If the patient has a "poor" appetite, (e.g. their `appetite_dummy` label is 0), 
    then the `pp_of_correct_label` shows the predicted probability of an 
    `appetite_dummy` score of 0. 
    
There's a fancy short-cut to the several lines of code in the cell above, that relies 
on the fact that multiplying by 0 gives 0.  We can form the `pp_of_correct_label` column 
by first multiplying the 0 / 1 label column values by the values in `pp_of_1`.  
This correctly sets the `pp_of_label` values for labels of 1, and sets the
...
pos_ps = appetite_dummy * pp_of_1
# Set label == 0 values to 1-predicted p, others to 0
neg_ps = (1 - appetite_dummy) * pp_of_0
# Combine by adding.
final = pos_ps + neg_ps
# This gives the same result as the code cell above:
hgb_predicted['pp_of_correct_label_fancy'] = final
hgb_predicted
```
...
final_again = appetite_dummy * pp_of_1 + (1 - appetite_dummy) * (1 - pp_of_1)
hgb_predicted['pp_again'] = final_again
hgb_predicted
```

When the `pp_of_correct_label` values are near 1, this means the result was close to 
the prediction.  When they are near 0, it means the result was unlike the prediction.

The scatterplot below shows this:

```{python}
good_appetites = hgb_predicted[hgb_predicted['appetite_dummy'] == 1].copy()
poor_appetites = hgb_predicted[hgb_predicted['appetite_dummy'] == 0].copy()
plt.scatter(good_appetites['pp_of_1'], good_appetites['pp_of_correct_label'], color = 'red', label = 'good appetite (1)')
plt.scatter(poor_appetites['pp_of_1'], poor_appetites['pp_of_correct_label'], color = 'blue', label = 'poor appetite (0)')
plt.xlabel('Predicted probability of being in class 1')
plt.ylabel('Likelihood')
plt.legend(loc = 'upper center' );
```
Remember that each point on the scatterplot above represents one patient.

From the graph you can see that, for each patient:

*If the prediction is good, then the likelihood score is high e.g.:*

* If the patient's appetite is 'good', and the predicted probability is near 1, then the likelihood is high.

* If the patient's appetite is 'poor', and the predicted probability is near 0, then the likelihood is high.

*If the prediction is bad, then the likelihood score is low e.g.:*

* If the patient's appetite is 'good', but the predicted probability is near 0, then the likelihood is low.

* If the patient's appetite is 'poor', but the predicted probability is near 1, then the likelihood is low.
...
`pp_of_correct_label`.  When the probabilities in `pp_of_correct_label` are all fairly near 1,
...
likelihood = np.prod(pp_of_correct_label)
likelihood
```
...
tend to be a reasonable number of small p values in `pp_of_correct_label`.  The
...
mr_ML = minimize(simple_ml_logit_cost,  # Cost function
                 ...
                 args=(hemoglobin, appetite_dummy),  # x and y values
...
```{python}
print('Likelihood with product of array', np.prod(pp_of_correct_label))
# The log-add-unlog version.
logs_of_pp = np.log(pp_of_correct_label)
...
[monotonically](https://en.wikipedia.org/wiki/Monotonic_function) with the
`likelihood`.  Consider any two pairs of (intercept, slope). Call the first
pair `p1` and the second pair `p2`.  Call the likelihoods for `p1` and `p2`:
`L2` and `L2` respectively.  Say we've got another way of calculating a value
for `p1` and `p2`, and the values that come back are `M1` and `M2`
respectively.  We can swap this other way into our cost function as long as
it is always true that: when `L1 > L2` then `M1 > M2`; when `L1 < L2` then `M1
< M2`; and when `L1 == L2`, `M1 == M2`.  In that case we call our new
calculation generating `M1` and `M2` *monotonic* with respect to the original
calculation generating `L1` and `L2`.  In our case this original calculation is
likelihood. The new monotonic calculation is the *log likelihood*.

You may be able to see that if the likelihood goes up, then the log of
likelihood will also go up, and vice versa, so the log of the likelihood is monotonic with respect to the likelihood, and we can use it instead of the likelihood, in our cost function.

This means that our cost function does not have to do the last nasty unlog step
above, that generates the tiny number.  We can just return the (minus of the)
log of the likelihood.  The log of the likelihood turns out to be a manageable
negative number, even when the resulting likelihood is tiny.
...
## Interpreting the intercept and slope in logistic regression

Having the predicted probabilities is very useful. But we do not *only* care about 
prediction, we also care about interpretation and explanation. 

Recall that, in linear regression, the intercept tells us the predicted value of 
the outcome variable when the value of the predictor(s) is 0. Also, in linear regression,
the slope of a predictor tells us the predicted average change in the outcome variable for
a one-unit change in the predictor. 

For instance, if we use linear regression to predict `Reb Blood Cell Count` from `Age`:

```{python}
import scipy.stats as sp

# linear regression with scipy
scipy_lin_reg = sp.linregress(df['Age'], df['Red Blood Cell Count'])

# do not worry about this code, it just generates the plot
y_hat = scipy_lin_reg[1] + scipy_lin_reg[0] * df['Age']
plt.scatter(df['Age'], df['Red Blood Cell Count'])
plt.plot(df['Age'], y_hat, color = 'red', label = 'Linear Regression Predictions')
plt.legend(loc = 'upper left')
plt.xlim([0, np.max(df['Age'])+5])
plt.xlabel('Age')
plt.ylabel('Red Blood Cell Count')
plt.show();
                              
# show the linear regression coefficients (intercept, slope...)
print('Scipy Linear Regression Intercept =', scipy_lin_reg[1])  
print('Scipy Linear Regression Slope =', scipy_lin_reg[0])   
```

The intercept tell us the predicted value of `Red Blood Cell Count` when `Age` equals 0. 
We can see that if we follow the trajectory of the prediction line, it will cross the y
axis at around 5.6-ish . This is probably not a sensible prediction, as it extrapolates beyond 
the data we have, as the youngest patient in our dataset is 6. But, theoretically we
interpret this intercept as the predicted `Red Blood Cell Count` for a newborn (`Age` == 0).

The slope, tells us how much `Red Blood Cell Count` changes for a 1 unit increase in 
`Age`.So, the slope tells us, if we had two new patients of different ages, how different 
would we predict their `Red Blood Cell Count` to be, based on their difference in age? 

Becausethe slope is negative, we would expect the older patient to have a lower 
`Red Blood Cell Count`. The value of the slope tells us we would expect `Red Blood Cell Count` to be 
-0.0159 lower for each one year increase in age.

Let's say one patient was 10 years older than the other:

```{python}
# age of the first new patient (20 years old)
age_new_patient_1 = 20

# age of the second new patient (40 years old)
age_new_patient_2 = 40

# the difference in age between the two patients
diff_age = age_new_patient_2 - age_new_patient_1

# the slope for age
age_slope = scipy_lin_reg[0]

#the predicted difference in Red Blood Cell Count, based on the age difference
print('Predicted difference in Red Blood Cell Count = ', age_slope * diff_age)
```

What do the slope and intercept in logistic regression tell us? 
...
Remember, that the line we fit in logistic regression predicts the <i> log odds ratio </i>.
In this case, we were predicting `Appetite` from `Hemoglobin`.

In logistic regression we interpret the intercept and slope in the same way as for linear regression, BUT 
the line they relate to is on the log odds ratio scale:
...
plt.title('Logit prediction')
plt.xlabel('Hemoglobin')
plt.ylabel('Log odds-ratio of "Good Appetite"');
```

The logistic regression intercept of -7.2918726387296235 tells us that, for a `Hemoglobin` score of 0,
the predicted log odds ratio of "good appetite" is -7.2918726387296235. We can see
that this is consistent with the graph above. The graph below extends the prediction line,
to show the intercept:

```{python}
hemoglobin_origin = np.append(hemoglobin, np.linspace(0, np.min(hemoglobin)))
predictions_or_log_origin = inter_MLL + slope_MLL * hemoglobin_origin 
plt.scatter(hemoglobin, predictions_or_log, color='gold')
fine_y_or_log = np.log(fine_y_or)
plt.plot(hemoglobin_origin, predictions_or_log_origin, linewidth=1, linestyle=':')
plt.title('Logit prediction')
plt.xlabel('Hemoglobin')
plt.ylabel('Log odds-ratio of "Good Appetite"');
```

We can see that the line does in fact cross the y axis at -7.2918726387296235

We can also verify this by using the intercept and slope to calculate the predicted 
log odds ratio of  a `Hemoglobin` score of 0:

```{python}
hemo_of_zero = 0
pred_log_or_0 = inter_MLL + slope_MLL * hemo_of_zero
pred_log_or_0
```

The logistic regression slope of 0.7991544178732585 tells us the average change in the 
log odds ratio of "good" appetite, for a one-unit change in `Hemoglobin`. As the slope is
positive, the predicted log odds ratio of good appetite increases by 0.7991544178732585
for each one unit increase in `Hemoglobin`.

This all makes sense on the log odds ratio scale. But it is not very interpretable.
It is not very practically meaningful for a doctor to say to a patient "given your
hemoglobin level, the predicted log odds of you having a good appetite are low".

Fortunately, we can transform the slope and intercept back to odds ratios (rather than log odds ratios), using
the conversion formula we used earlier. We do have to be careful with our interpretations
when we do this, especially with negative intercepts and slopes. 

To convert the intercept and slope - so that they relate to the odds ratio of
having "good appetite", rather than to the log odds ratio - we raise e to the power of
the coefficient we want to convert. Let's convert the intercept first:
...
This means the predicted odds ratio of having a "good" appetite - for a patient with
a `Hemoglobin` score of 0 - is 0.000681051494457782

We can see that is is consistent with the graph of the predicted odds ratios:
...
We can see that the predicted odds ratio is close to 0 for `Hemoglobin` scores around 10, and it continues
getting lower as `Hemoglobin` decreases. So an odds ratio of 0.000681051494457782 for a `Hemoglobin` score
of 0 seems plausible.

When we are interpreting the intercept, we can convert it to a probability, in the same way as earlier:

*Note:* we <b> cannot </b> do this last step with the slope, as we will see.

```{python}
# probability = (odds ratio)/(1 + odds ratio)
np.exp(inter_MLL)/(1 + np.exp(inter_MLL))
```

The predicted probability of having a "good" appetite - for a patient with
a `Hemoglobin` score of 0 - is 0.0006805879789975758

We can see that this is consistent with the sigmoid of the predicted probabilities:
...
The predicted probability of "good" appetite is clearer near to 0 at around 3.  

To convert the slope, we follow the same process:

```{python}
np.exp(slope_MLL)
```

Recall that before this transformation the slope tells us the predicted
change in the log odds ratio for a one unit change in the predictor.

The transformed slope tells us the predicted change in the odds ratio
...
(formulation from https://www.juanbattle.com/files/2012/04/Regression-Logistic-4.pdf)

In the current example, this means that the odds of "good" appetite gets, on average, 2.223659846277835 times
larger for every one unit increase in `Hemoglobin`. Let's check this, by calculating
the predicted change in the odds ratio for a `Hemoglobin` score of 15, relative to a
`Hemoglobin` score of 16.

```{python}
# predicted odds ratio for hemoglobin score of 15
hemo_15 = 15
or_15 = np.exp(inter_MLL + slope_MLL * hemo_15)
or_15
```

```{python}
# predicted odds ratio for hemoglobin score of 16
hemo_16 = 16
or_16 = np.exp(inter_MLL + slope_MLL * hemo_16)
or_16
```

When we are considering the slope, we are considering the ratio of two odds ratios:

```{python}
# odds ratio of hemoglobin == 16 divided by odds ratio of hemoglobin == 15
or_16/or_15
```

The odds ratio for the hemoglobin score of 16 is 2.223659846277835 time as large 
as the odds ratio for the hemoglobin score of 15, as the slope told us it would be.

Again, we can see that this is consistent with the graph of the predicted odds ratios:

```{python}
# do not worry about this code, it just creates the plot
...
plt.plot(fine_x, fine_y_or, linewidth=1, linestyle=':')
plt.title('Odds-ratio prediction')
plt.xticks(np.arange(int(np.min(hemoglobin)), int(np.max(hemoglobin)+1), step = 1))
plt.yticks(np.arange(int(np.min(predictions_or)), int(np.max(predictions_or)+1), step = 50))
plt.xlabel('Hemoglobin')
plt.ylabel('Odds-ratio');
```

It is clearer for higher values of `Hemoglobin`, be we can see that the
odds ratio is approximately doubling for every 1 unit increase in `Hemoglobin`.

We cannot express the slope as a probability, in the same way as we can for the intercept.
This is because the odds ratio the slope relates to is a ratio of two odds ratios:

$ e^{\text{slope}} = \frac{\text{odds ratio of event of interest AFTER one unit change in predictor}}{\text{odds ratio of event of interest BEFORE one unit change in predictor}}$

Because of this, the probability conversion does not work in the same way (for more information on this, see
this post: https://stats.stackexchange.com/questions/363791/logit-interpreting-coefficients-as-probabilities).

However, we an interpret the slope in terms of a percentage change in the odds ratio, for a one unit
increase in the predictor. We do this as follows:

```{python}
(np.exp(slope_MLL) - 1) * 100
```

This tells us that the odds ratio of "good" appetite incrases by 122% for each one unit increase in `Hemoglobin`.
This percentage change relates to how the odds ratio changes as the predictor changes.

It is important to distinguish this percentage change in the odds ratio from the predicted probabilities on the
sigmoid.

The predicted probabilities on the sigmoid (shown below) and the predicted probability of "good" appetite
for each patient, as a function of their `Hemoglobin` score:
...
* Consider the expression `np.sum(np.log(pp_of_correct_label))` in the
  ...
  `np.sum(np.log10(pp_of_correct_label))` (log to the base 10).  What would happen to:
