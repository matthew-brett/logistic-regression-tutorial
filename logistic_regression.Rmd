...
The logit transformation from the sigmoid curve to the straight line is:

* Convert probability to odd-ratios.
* Take the log of the result.

Our full logit transformation is therefore:
...
Here's the original sigmoid predictions and the predictions with the `logit` transform applied:
...
vals_back = np.exp(np.log(original_vals))
vals_back
```
...
of squared prediction errors from the line.  But the cost function can do anything it likes to generate a score from the line.
...
For example, let's say I want to get a score for the intercept -7 and the slope
0.8.

First I get the straight-line predictions in the usual way:
...
These are predictions on the straight line, but I now need to transform them to p value (0 to 1) predictions on the sigmoid.  I use `inv_logit`:
...
Finally, I want to compare the predictions to the actual data to get a score.  One way I could do this is our good old sum of squares difference between the sigmoid p-value predictions and the 0 / 1 values:
...
To build my cost function, first I make a function to calculate my predicted p values from a slope and intercept and the x-axis values we are predicting from:
...
Next I make the cost function that minimize will use.  It must accept an array
...
There's a fancy short-cut to the several lines above.  We can form the
`pp_of_label` column by first multiplying the 0 / 1 label column by the
`pp_of_1`.  This correctly sets the `pp_of_label` values for labels of 1, and
sets the remainder to 0.  Then we reverse the 0 / 1 labels by subtracting from
1, and multiply by `pp_of_0` to set the 0 values to their correct values.
...
tend to be a fairly large number of small p values in `pp_of_labels`.  The
question is - do other values for the intercept and slope give smaller or
larger values for the likelihood?

We can put this *likelihood* scoring into our cost function, instead of using sum of squares.

One wrinkle is that we want our cost function value to be *lower* when the line
is good, but the likelihood is *higher* when the line is a good one.  We solve
this by sticking a minus on the likelihood before we return from the cost
function. This makes `minimize` find the parameters giving the *minimum* of the
*negative likelihood*, and therefore, the *maximum likelihood* (ML).
...
different plausible lines to differ by an even small amount.  By default,
`minimize` will treat these very small differences as insignificant.  We can tell it to pay attention to these kind of differences by passing a very small value to the `tol` parameter (`tol` for *tolerance*).
...
`minimize` is complaining that it was not happy with the numerical accuracy of its answer, and we will address that soon.  For now, lets look at the predictions to see if they are reasonable.
...
As you've seen, the likelihood values can be, and often are, very small, and close to zero.

You may also know standard numerical calculations on computers are not
...
The combination of low likelihood values, and limited calculation precision can
be a problem for the simple ML logistic cost function you see above.  As a
...
What you see here is that we get the same answer if we:
...
We can replace multiplication by addition, if we take the logs of the values.

Here we do the same log-then-add-then-unlog trick on a array of numbers:
...
We can use this trick to calculate our likelihood by adding logs instead of multiplying the probabilities directly:
...
it's always true that: when `L1 > L2` then `M1 > M2`; when `L1 < L2` then `M1 <
M2`; and when `L1 == L2`, `M1 == M2`.  In that case we call our new calculation
generating `M1` and `M2` *monotonic* with respect to the original calculation
generating `L1` and `L2`.  In our case this original calculation is likelihood.
The new monotonic calculation is the *log likelihood*.

You may be able to see that if the likelihood goes up, then the log of
likelihood will also go up, and vice versa.

This means that, for our cost function, we don't have to do the last nasty
unlog step above, that generates the tiny number.  We can just return the
...
This trick gives us a much more stable way of returning something from the cost function.  That something will vary in the same as the likelihood - but it will no longer suffer from the calculation errors we expect from very small numbers:

```{python}
def mll_logit_cost(intercept_and_slope, x, y):
    """ More stable version of cost function for maximum likelihood
...
The values that come back are very similar to the more fragile version using the
likelihood:
...
Notice Statsmodels lists the "Model" as "Logit" and the "Method" as "MLE" -
Maximum Likelihood Estimation.

Look at the table above under 'coef'. Compare the logistic regression intercept
and slope that Statsmodels has obtained to the ones we obtained through using
`minimize`:
...
Let's plot the predicted probabilities of having "good" appetite, from
Statsmodels:
...
Let's see what the largest absolute difference between the predictions from the
two methods is:
