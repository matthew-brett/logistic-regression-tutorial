...
refresher](logarithms_refreshed) page).  You have seen above that there is a
simple formula to go from odds ratios to probabilities.  The transformation
that *reverses*  (inverts) the logit transform is therefore:

```{python}
def inv_logit(v):
    """ Reverse logit transformation on array of transformed values `v`
    """
...
First we convince ourselves that `inv_logit` reverses `logit`, and vice versa.
